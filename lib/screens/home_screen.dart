import 'package:nytapp/helpers/responsive_helpers.dart';
import 'package:nytapp/models/article_model.dart';
import 'package:nytapp/services/api_services.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  _fetchArticles() async {
    List<Article> articles = await APIService().fetchArticlesBySection(
        'technology');
    setState(() {
      _articles = articles;
    });
  ...
  _buildArticlesGrid(MediaQueryData mediaQuery) {
    List<GridTile> tiles = [];
    _articles.forEach(
            (article) => tiles.add(_buildArticleTile(article, mediaQuery))
    );
    return Padding(
